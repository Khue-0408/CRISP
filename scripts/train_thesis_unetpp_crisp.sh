...
if crisp_should_use_tiny_smoke_data "$@"; then
  ...
  EXTRA_ARGS=(
    ...
    "source_data.local_split.cache_dir=/tmp/crisp_thesis_smoke_splits_unetpp_crisp"
    "eval_datasets=[Kvasir]"
    "output_dir=/tmp/crisp_thesis_unetpp_crisp_out"
    "eval_output_dir=/tmp/crisp_thesis_unetpp_crisp_eval"
...
"$PYTHON_BIN" -m crisp.scripts.train \
  ...
  --config-name experiment/thesis_unetpp_crisp \
  "${EXTRA_ARGS[@]}" \
  "$@"
