...
CHECKPOINT_PATH="${1:?Usage: bash scripts/eval_thesis_unet.sh /path/to/checkpoint.pt [hydra overrides...]}"
shift
CONFIG_NAME="${CRISP_EVAL_CONFIG:-experiment/thesis_unet_crisp}"
if [[ -z "${CRISP_EVAL_CONFIG:-}" && "$CHECKPOINT_PATH" == *baseline* ]]; then
  CONFIG_NAME="experiment/thesis_unet_baseline"
fi
...
if [[ -z "${CRISP_DATA_ROOT:-}" ]] && ! crisp_arg_present "source_data.root=" "$@"; then
  ...
  EXTRA_ARGS=(
    ...
    "eval_output_dir=/tmp/crisp_thesis_unet_eval"
  )
fi
