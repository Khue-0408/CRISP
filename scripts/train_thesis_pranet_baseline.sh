...
if crisp_should_use_tiny_smoke_data "$@"; then
  ...
  EXTRA_ARGS=(
    ...
    "source_data.local_split.cache_dir=/tmp/crisp_thesis_smoke_splits_pranet_baseline"
    "eval_datasets=[Kvasir]"
    "output_dir=/tmp/crisp_thesis_pranet_baseline_out"
    "eval_output_dir=/tmp/crisp_thesis_pranet_baseline_eval"
  )
fi

"$PYTHON_BIN" -m crisp.scripts.train \
  --config-path "$ROOT_DIR/configs" \
  --config-name experiment/thesis_pranet_baseline \
  "${EXTRA_ARGS[@]}" \
  "$@"
