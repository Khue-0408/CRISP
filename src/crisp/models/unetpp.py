...
class UNetPP(BaseSegmentationModel):
    ...
    def __init__(
        ...
        **_: object,
        ...
        super().__init__()
        input_channels = int(in_channels if in_channels is not None else input_channels)
        ...
        self._decoder_channels = nb_filter[2]

    @property
    def decoder_channels(self) -> int:
        return self._decoder_channels

    def forward(self, input: torch.Tensor) -> SegmentationOutput:
        ...
        if self.deep_supervision:
            ...
            logits = outputs[-1]
            aux = {"deep_supervision": outputs}
        else:
            logits = self.final(x0_4)
            aux = None

        return SegmentationOutput(logits=logits, features=x2_2, aux=aux)
